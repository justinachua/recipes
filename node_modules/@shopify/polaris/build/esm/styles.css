@charset "UTF-8";
:root { --polaris-version-number: '9.4.0'; }
...
.Polaris-Badge--statusAttention { --pc-badge-component-badge-pip-color: var(--p-icon-attention); background-color: var(--p-surface-attention); color: var(--p-text); }
...
.Polaris-TextField__VerticalContent { position: relative; z-index: var(--pc-text-field-contents); color: var(--p-text-subdued); -webkit-user-select: none; user-select: none; padding: var(--p-space-2) var(--p-space-3) var(--p-space-2) var(--p-space-2); max-height: 8.75rem; overflow: scroll; border: var(--p-border); width: 100%; }

.Polaris-TextField__VerticalContent > .Polaris-TextField__Input { padding-left: var(--p-space-0); padding-bottom: var(--p-space-0); }
...
.Polaris-Banner { --pc-banner-secondary-action-horizontal-padding: var(--p-space-3); --pc-banner-secondary-action-vertical-padding: calc(var(--p-line-height-6)*0.5 - var(--p-line-height-2)*0.5); position: relative; display: flex; }
...
.Polaris-DataTable__IncreasedTableDensity .Polaris-DataTable__Cell--sortable { padding: var(--p-space-2); }
